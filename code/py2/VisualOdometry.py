...
def testVisualOdometry(n_points=30):
	...
	# Rotation matrix:
	phi = 0.1*(np.random.random(1)*2-1) * numpy.pi;
	theta = 0.1*(np.random.random(1)*2-1) * numpy.pi;
	psi = 0.1*(np.random.random(1)*2-1) * numpy.pi;
	...
	R_psi = np.zeros([3,3]);
	R_psi[1,1] = np.cos(phi);
	R_psi[1,2] = np.sin(phi);
	R_psi[2,1] = -np.sin(phi);
	R_psi[2,2] = np.cos(phi);
	...
	for p in range(n_points):
		points_world[p, :] = size * (np.rand(1,3)*2-np.ones([1,3])) + transl;
